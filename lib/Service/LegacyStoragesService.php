...
abstract class LegacyStoragesService {
	...
	protected function populateStorageConfigWithLegacyOptions(
		...
		if ($mountType === \OCA\Files_External1\MountConfig::MOUNT_TYPE_USER) {
			$applicableUsers = $storageConfig->getApplicableUsers();
			if ($applicable !== 'all') {
				$applicableUsers[] = $applicable;
				$storageConfig->setApplicableUsers($applicableUsers);
			}
		} elseif ($mountType === \OCA\Files_External1\MountConfig::MOUNT_TYPE_GROUP) {
		...
		foreach ($mountPoints as $mountType => $applicables) {
			foreach ($applicables as $applicable => $mountPaths) {
				foreach ($mountPaths as $rootMountPath => $storageOptions) {
					...
					$storageOptions['options'] = \OCA\Files_External1\MountConfig::decryptPasswords($storageOptions['options']);
					...
					if (isset($storageOptions['id'])) {
						...
						$configId = \OCA\Files_External1\MountConfig::makeConfigHash($storageOptions);
