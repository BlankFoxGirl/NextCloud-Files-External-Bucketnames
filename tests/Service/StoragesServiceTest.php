...
abstract class StoragesServiceTest extends \Test\TestCase {
	...
	protected function setUp(): void {
		...
		$this->backendService =
			$this->getMockBuilder('\OCA\Files_External1\Service\BackendService')
		...
		$authMechanisms = [
			...
			'identifier:\OCA\Files_External1\Lib\Auth\NullMechanism' => $this->getAuthMechMock(),
		];
		...
		$sftpBackend = $this->getBackendMock('\OCA\Files_External1\Lib\Backend\SFTP', '\OCA\Files_External1\Lib\Storage\SFTP');
		$backends = [
			'identifier:\OCA\Files_External1\Lib\Backend\DAV' => $this->getBackendMock('\OCA\Files_External1\Lib\Backend\DAV', '\OC\Files\Storage\DAV'),
			'identifier:\OCA\Files_External1\Lib\Backend\SMB' => $this->getBackendMock('\OCA\Files_External1\Lib\Backend\SMB', '\OCA\Files_External1\Lib\Storage\SMB'),
			'identifier:\OCA\Files_External1\Lib\Backend\SFTP' => $sftpBackend,
			'identifier:sftp_alias' => $sftpBackend,
		];
		$backends['identifier:\OCA\Files_External1\Lib\Backend\SFTP']->method('getLegacyAuthMechanism')
		...
		$containerMock->method('query')
			->willReturnCallback(function ($name) {
				if ($name === 'OCA\Files_External1\Service\BackendService') {
	...
	public function testDeleteStorage($backendOptions, $rustyStorageId) {
		$backend = $this->backendService->getBackend('identifier:\OCA\Files_External1\Lib\Backend\DAV');
		$authMechanism = $this->backendService->getAuthMechanism('identifier:\Auth\Mechanism');
		$storage = new StorageConfig(255);
		$storage->setMountPoint('mountpoint');
		$storage->setBackend($backend);
	...
	public function testCreateStorageInvalidAuthMechanismClass() {
		$storage = $this->service->createStorage(
			'mount',
			'identifier:\OCA\Files_External1\Lib\Backend\SMB',
