...
export const getContents = async (): Promise<ContentsWithRoot> => {
	const response = await axios.get(generateOcsUrl('apps/files_external1/api/v1/mounts')) as AxiosResponse<OCSResponse<MountEntry>>
...
export const getStatus = function(id: number, global = true) {
	...
	return axios.get(generateUrl(`apps/files_external1/${type}/${id}?testOnly=false`)) as Promise<AxiosResponse<StorageConfig>>
}
