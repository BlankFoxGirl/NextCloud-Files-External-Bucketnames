...
export const action = new FileAction({
	...
	async exec(node: Node) {
		...
		const response = await fetch(generateOcsUrl('/apps/files_external1/api/v1/auth'), {
		...
		if (data.ocs.data.user && data.ocs.data.password) {
			const configResponse = await axios.put(generateUrl('apps/files_external1/userglobalstorages/{id}', node.attributes), {
